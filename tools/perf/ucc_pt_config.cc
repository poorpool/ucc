...
ucc_pt_config::ucc_pt_config()
{
    ...
    bench.root_shift          = 0;
    bench.mult_factor         = 2;
    bench.oneside_buffer_size = 0;
    comm.mt                   = bench.mt;
    comm.oneside_buffer_size  = bench.oneside_buffer_size; // cyx add
}
...
const std::map<std::string, ucc_datatype_t> ucc_pt_datatype_map = {
    {"int8", UCC_DT_INT8},        {"uint8", UCC_DT_UINT8},
    {"int16", UCC_DT_INT16},      {"uint16", UCC_DT_UINT16},
    {"float16", UCC_DT_FLOAT16},  {"bfloat16", UCC_DT_BFLOAT16},
    {"int32", UCC_DT_INT32},      {"uint32", UCC_DT_UINT32},
    {"float32", UCC_DT_FLOAT32},  {"float32_complex", UCC_DT_FLOAT32_COMPLEX},
    {"int64", UCC_DT_INT64},      {"uint64", UCC_DT_UINT64},
    {"float64", UCC_DT_FLOAT64},  {"float64_complex", UCC_DT_FLOAT64_COMPLEX},
    {"int128", UCC_DT_INT128},    {"uint128", UCC_DT_UINT128},
    {"float128", UCC_DT_FLOAT64}, {"float128_complex", UCC_DT_FLOAT128_COMPLEX},
};
...
ucc_status_t ucc_pt_config::process_args(int argc, char *argv[])
{
    ...
    while ((c = getopt(argc, argv, "c:b:e:d:m:n:w:o:N:r:S:O:iphFT")) != -1) {
        switch (c) {
        case 'c':
            if (ucc_pt_op_map.count(optarg) == 0) {
                std::cerr << "invalid operation: " << optarg << std::endl;
        ...
        case 'd':
            if (ucc_pt_datatype_map.count(optarg) == 0) {
                std::cerr << "invalid datatype:" << optarg << std::endl;
                return UCC_ERR_INVALID_PARAM;
            }
        ...
        case 'O': // cyx add
            st =
                ucc_str_to_memunits(optarg, (void *)&bench.oneside_buffer_size);
            if (st != UCC_OK) {
                std::cerr << "failed to parse one side buffer size"
                          << std::endl;
                return st;
            }
            comm.oneside_buffer_size = bench.oneside_buffer_size;
            break;
        case 'h':
        default:
            print_help();
...
void ucc_pt_config::print_help()
{
    ...
    std::cout
        << "  -f <number>: multiplication factor between sizes. Default : 2."
        << std::endl;
    std::cout << "  -O: <bytes> oneside buffer size (calculated by count, "
                 "datatype, mpi_size. cyx add)"
              << std::endl;
    ...
    std::cout << "  -S: <number>: root shift for rooted collectives"
              << std::endl;
