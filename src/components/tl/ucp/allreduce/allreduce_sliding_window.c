...
static inline void ucc_tl_ucp_allreduce_sliding_window_reset_buf(
    ucc_tl_ucp_allreduce_sw_buf_t *buf)
{
    buf->state   = FREE;
    buf->count   = 0;
    buf->bytes   = 0;
...
ucc_tl_ucp_allreduce_sliding_window_start(ucc_coll_task_t *coll_task)
{
    ...
    ucc_rank_t            put_window_size =
        UCC_TL_UCP_TEAM_LIB(team)->cfg.allreduce_sliding_window_put_window_size;
    ucc_tl_ucp_allreduce_sw_global_work_buf_info_t *gwbi_p =
        coll_args->args.global_work_buffer;
    ucc_tl_ucp_task_t *rdma_task =
        ucc_derived_of(schedule->tasks[0], ucc_tl_ucp_task_t);
    ...
    if (!inplace) {
        ...
        ucc_assert(rdma_task->allreduce_sliding_window.bufs->src_ebuf
                       ->packed_key_len <= ALLREDUCE_PACKED_KEY_MAX_LEN);
        memcpy(
            allgather_data->packed_src_key,
    ...
    pipe->my_offset =
        ucc_buffer_block_offset(count_total * dt_size, size, rank);
...
out:
    tl_error(UCC_TASK_LIB(rdma_task),
             "failed to start allreduce sliding window: %s",
             ucc_status_string(status));
    return status;
}
...
static inline void
ucc_tl_ucp_allreduce_sliding_window_free_rkeys(ucc_coll_task_t *coll_task)
{
...
static ucc_status_t ucc_tl_ucp_allreduce_sliding_window_rdma_task_finalize(
    ...
    ucc_tl_ucp_allreduce_sw_buf_t *getbuf)
{
    ...
    status = ucc_dt_reduce(accbuf->buf, getbuf->buf, accbuf->buf, accbuf->count,
                           dt, args, 0, 0, exec,
    ...
    ucc_coll_task_t *coll_task)
{
    ...
    UCC_CHECK_GOTO(
        ucc_tl_ucp_allreduce_sliding_window_allgather_info_finalize(task), err,
        status);
...
err:
    ucc_tl_ucp_allreduce_sliding_window_free_task(coll_task);
    ucc_tl_ucp_allreduce_sliding_window_free_pipe(coll_task);
    tl_error(coll_task->team->context->lib, "key exchange failure: %s",
             ucc_status_string(status));
    goto out;
}
...
static inline void ucc_tl_ucp_allreduce_sliding_window_mark_redbuf_free(
    ...
    ucc_tl_ucp_allreduce_sw_buf_t *redbuf, ucc_rank_t host_team_size)
{
    redbuf->state = FREE;
    pipe->avail_buffs++;
    pipe->red_idx++;
...
void ucc_tl_ucp_allreduce_sliding_window_rdma_progress(
    ucc_coll_task_t *coll_task)
{
    ucc_tl_ucp_task_t *task    = ucc_derived_of(coll_task, ucc_tl_ucp_task_t);
    ...
    if (pipe->count_serviced < pipe->my_count) {
        if ((pipe->count_received < pipe->my_count) &&
            ...
            src_addr =
                PTR_OFFSET(task->allreduce_sliding_window.bufs->sbufs[src_rank],
                           get_offset);
            dst_addr = getbuf->buf;
            ...
            getbuf->ucp_req = ucp_get_nbx(
                ep, dst_addr, data_size, (uint64_t)src_addr,
        ...
        if ((pipe->count_serviced < pipe->count_reduced) &&
            ...
            window =
                ucc_min(put_window_size, host_team_size - pipe->posted_put);
            ...
            for (i = 0; i < window; i++) {
                ...
                dst_addr = PTR_OFFSET(
                    ...
                    put_offset);
                put_idx = pipe->posted_put % put_window_size;
                ...
                task->allreduce_sliding_window.put_requests[put_idx] =
                    ucp_put_nbx(ep, src_addr, data_size, (uint64_t)dst_addr,
                                task->allreduce_sliding_window.bufs
                                    ->dst_rkeys[dst_rank],
                                &req_param);
                                         ...
                                         ucc_coll_task_t     **task_h)
{
    ...
    *task_h                  = &schedule->super;
    ...
    if (ucc_tl_ucp_allreduce_sliding_window_alloc_pipe(team, rdma_task) !=
        UCC_OK) {
    ...
    rdma_task->super.progress =
        ucc_tl_ucp_allreduce_sliding_window_rdma_progress;
    rdma_task->super.finalize =
        ucc_tl_ucp_allreduce_sliding_window_rdma_task_finalize;

    UCC_CHECK_GOTO(
        ucc_tl_ucp_allgather_ring_init(
            &bargs, team, &rdma_task->allreduce_sliding_window.allgather_task),
    ...
    UCC_CHECK_GOTO(ucc_schedule_add_task(schedule, &rdma_task->super), out,
                   status);
    UCC_CHECK_GOTO(ucc_event_manager_subscribe(
                       &schedule->super, UCC_EVENT_SCHEDULE_STARTED,
                       &rdma_task->super, ucc_task_start_handler),
                   free_barrier_task, status);

    UCC_CHECK_GOTO(ucc_schedule_add_task(schedule, barrier_task), out, status);
    UCC_CHECK_GOTO(
        ucc_event_manager_subscribe(&rdma_task->super, UCC_EVENT_COMPLETED,
                                    barrier_task, ucc_task_start_handler),
        free_barrier_task, status);
